...
async def test_reward_function(mock_make_openai_request, mock_get_transcript,
    ...
    mock_responses_pattern = [
        ...
        MockResponse(meets_brief=True),          # Video 1 brief 1 check
        MockResponse(meets_brief=False),         # Video 1 brief 2 check
        MockResponse(injection_detected=False),  # Video 2 injection check
        MockResponse(meets_brief=True),          # Video 2 brief 1 check
        MockResponse(meets_brief=True),          # Video 2 brief 2 check
        MockResponse(injection_detected=False),  # Video 3 injection check
        MockResponse(meets_brief=False),         # Video 3 brief 1 check
        MockResponse(meets_brief=False),         # Video 3 brief 2 check
        MockResponse(injection_detected=False),  # Video 4 injection check
        MockResponse(meets_brief=False),         # Video 4 brief 1 check
        MockResponse(meets_brief=True),          # Video 4 brief 2 check
    ]
