...
def validate_curve_scoring_periods() -> bool:
    """
    Validate that the curve scoring periods are calculated correctly.
    
    This is a utility function for testing and debugging to ensure
    the date calculations are correct.
    
    Returns:
        True if period calculations are valid
    """
    try:
        today = datetime.now()
        
        # Calculate periods (same logic as main function)
        day1_start_offset = YT_REWARD_DELAY + YT_ROLLING_WINDOW - 1
        day1_end_offset = YT_REWARD_DELAY
        day2_start_offset = YT_REWARD_DELAY + YT_ROLLING_WINDOW - 2
        day2_end_offset = YT_REWARD_DELAY - 1
        
        day1_start = today - timedelta(days=day1_start_offset)
        day1_end = today - timedelta(days=day1_end_offset)
        day2_start = today - timedelta(days=day2_start_offset)
        day2_end = today - timedelta(days=day2_end_offset)
        
        # Validate period lengths
        day1_length = (day1_end - day1_start).days + 1
        day2_length = (day2_end - day2_start).days + 1
        
        if day1_length != YT_ROLLING_WINDOW:
            bt.logging.error(f"Day 1 period length incorrect: {day1_length} != {YT_ROLLING_WINDOW}")
            return False
            
        if day2_length != YT_ROLLING_WINDOW:
            bt.logging.error(f"Day 2 period length incorrect: {day2_length} != {YT_ROLLING_WINDOW}")
            return False
        
        # Validate overlap (should be YT_ROLLING_WINDOW - 1 days)
        overlap_start = max(day1_start, day2_start)
        overlap_end = min(day1_end, day2_end)
        overlap_days = (overlap_end - overlap_start).days + 1
        
        expected_overlap = YT_ROLLING_WINDOW - 1
        if overlap_days != expected_overlap:
            bt.logging.error(f"Period overlap incorrect: {overlap_days} != {expected_overlap}")
            return False
        
        bt.logging.debug(
            f"Curve scoring periods validated: "
            f"Day1={day1_start.strftime('%Y-%m-%d')} to {day1_end.strftime('%Y-%m-%d')}, "
            f"Day2={day2_start.strftime('%Y-%m-%d')} to {day2_end.strftime('%Y-%m-%d')}, "
        ...
        bt.logging.error(f"Error validating curve scoring periods: {e}")
        return False